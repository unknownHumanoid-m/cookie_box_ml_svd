# -*- coding: utf-8 -*-
"""
Train the "how many pulses" MLP on 1D-SVD h5 files.

Same model as `src/denoising/train_how_many.py` (SimpleMLP flattens with
`x.view(x.size(0), -1)`, so `input_size = np.prod(input_shape)` handles a
flat `(L,)` per-shot vector identically to a 2D `(H, W)` one). This folder
exists so 1D-SVD runs get their own SLURM scripts, checkpoints, and figure
directory.

Example
-------
    python3 train_how_many.py \
        --data_dirs /path/to/mrco_h5_svd/1d_svd_train/ \
        --input_key Ypdf \
        --min_pulses 1 --max_pulses 4 \
        --save_model how_many_svd1d_r8.pth
"""
