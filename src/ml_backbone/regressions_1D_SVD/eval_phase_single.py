# -*- coding: utf-8 -*-
"""
Evaluate the deep-MLP single-pulse phase regressor on autoencoder-denoised
1D-SVD h5 files, using the how_many MLP as an upstream filter.
...
-------
    python3 eval_phase_single.py \
        --data_dirs /path/to/svd1d_r8_autoencoder/test \
        --model_path /path/to/phase_single_mlp.pth \
        --how_many_path /path/to/how_many_denoised_svd1d_r8.pth
"""
...
class PhaseMLP(nn.Module):
    """Mirrors train_phase_single.PhaseMLP (dropout retained; .eval() disables it)."""
    ...
    for path in files:
        ...
        with h5py.File(path, "r") as f:
            for shot in f.keys():
                ...
                npulses.append(n)
    x = torch.from_numpy(np.stack(imgs))  # (N, L) — PhaseMLP flattens internally
...
def main():
    ...
    hidden = tuple(ckpt.get("hidden", (1024, 512, 512, 256, 128)))
    model = PhaseMLP(input_size=int(np.prod(input_shape)),
                     num_classes=num_classes, hidden=hidden).to(device)
    model.load_state_dict(ckpt["state_dict"])
    model.eval()
    print(f"phase-single regressor loaded: num_classes={num_classes}, "
          f"input_shape={input_shape}, hidden={hidden}, input_key={input_key}")
    ...
    with torch.no_grad():
        for imgs, phases_true, npulses in loader:
            ...
            # how_many uses the same flat input; PhaseMLP takes (N, L) directly.
