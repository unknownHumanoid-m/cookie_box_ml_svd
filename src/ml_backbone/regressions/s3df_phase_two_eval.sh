...
#SBATCH --job-name=phase_two_eval
...
# ----------------------------------------------------------------------------
# Eval-only: runs the already-trained 2D-SVD two-pulse regressor over every
...
# Identifiers match the layout written by s3df_phase_two_train.sh /
# s3df_how_many_train.sh:
#     phase_two_svd2d_r<R>               (denoised)
#     phase_two_noisy_svd2d_r<R>         (noisy)
...
run_eval () {
    ...
    if [ "$branch" = "denoised" ]; then
        phase_identifier="phase_two_svd2d_r${rank}"
        how_many_identifier="how_many_denoised_svd2d_r${rank}"
    else
        phase_identifier="phase_two_noisy_svd2d_r${rank}"
