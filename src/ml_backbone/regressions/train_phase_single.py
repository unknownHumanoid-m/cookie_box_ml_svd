# -*- coding: utf-8 -*-
"""
Train the ResNet18 single-pulse phase regressor on autoencoder-denoised 2D-SVD
h5 files.

Reads `Ypdf_denoised` (16, 512) from each shot and regresses the single phase
angle (0..2*pi). Only shots with `npulses == 1` are used for training.
Downstream, the how_many MLP decides which shots go through this regressor at
inference time.
...
-------
    python3 train_phase_single.py \
        ...
        --save_model phase_single_resnet18.pth
"""
...
# Bring in resnet18 from the sibling file.
...
def load_single_pulse_h5(paths, input_key="Ypdf_denoised"):
    ...
    x = torch.from_numpy(np.stack(inputs)).unsqueeze(1)  # (N,1,H,W)
