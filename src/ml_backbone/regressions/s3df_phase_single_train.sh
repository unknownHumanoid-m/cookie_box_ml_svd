...
# ----------------------------------------------------------------------------
# Retrain the 2D single-pulse phase regressor over every (rank, branch) pair
# that still holds pre-Option-A ckpts:
#     r=2 denoised, r=2 noisy, r=4 denoised, r=4 noisy, r=8 noisy
# r=8 denoised was already retrained at 14:49 today and is skipped.
#
# Each iteration writes to
#     $BASE_SAVE_DIR/<IDENTIFIER>/<IDENTIFIER>.pth
# where IDENTIFIER matches the layout the eval .sh already expects
# (e.g. phase_single_svd2d_r2, phase_single_noisy_svd2d_r2).
# ----------------------------------------------------------------------------
...
run_one () {
    ...
    if [ "$branch" = "denoised" ]; then
        identifier="phase_single_svd2d_r${rank}"
    else
        identifier="phase_single_noisy_svd2d_r${rank}"
    fi
    ...
    local cmd=(python3 train_phase_single.py
        --data_dirs "${train_dir}"
        --input_key "${input_key}"
