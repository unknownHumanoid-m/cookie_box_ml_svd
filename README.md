...
**FPGA quantization-aware training.** The whole point of the split
bottleneck is that the deployed subgraph is small; the natural next step
is QAT so the FC layers can be quantized cleanly. That's deferred until
the FPGA target bit width and DSP budget are decided — implementing it
now would just be work we throw away when the constraint numbers land.
The path we'd likely take: (1) run this stack float32 to get a baseline,
(2) freeze the recon branch and re-run task-only fine-tuning with
`torch.ao.quantization` fake-quant on the encoder trunk +
`bottleneck_task` + heads, (3) re-export.

Other things intentionally not in this project:

* No SVD anywhere. It was the source of the segment-magnitude imbalance
  problems, and the joint-trained bottleneck subsumes its role.
* No shared bottleneck. The two-bottleneck split is a hard design
  requirement, not a knob.
* No per-pulse-count regressors (the old `phase_single` /
  `phase_two` fan-out). The single phase head here is meant for the
  1-pulse case; extending to a multi-pulse head is a separate design
  decision (see the "arccos-of-cos" convention used in the 2-pulse
  regressor for how the repo handles the pulse-order ambiguity).
